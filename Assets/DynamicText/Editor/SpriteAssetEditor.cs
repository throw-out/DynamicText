...
namespace UnityEditor.UI
{
    ...
    internal class SpriteAssetEditor : Editor
    {
        private SpriteAsset component;
        ...
        private void OnEnable()
        {
            ...
            component = target as SpriteAsset;
        ...
        public override void OnInspectorGUI()
        {
            ...
            m_Data.isExpanded = EditorGUILayout.Foldout(m_Data.isExpanded, "Sprites");
                ...
                fallbacksList = new ReorderableList(serializedObject, m_Fallbacks, true, false, true, true)
                {
                    drawElementCallback = (rect, index, selected, focused) =>
                    {
                        if (component.Fallbacks == null || index >= component.Fallbacks.Count)
                            return;
                        var newFallback = EditorGUI.ObjectField(rect, component.Fallbacks[index], typeof(SpriteAsset), false) as SpriteAsset;
        ...
        bool RendererDataMenu()
        {
            ...
            {
                component.Data.Add(GetDefaultKey(), new SpriteAsset.SpriteData());
                dirty = true;
            }
            if (GUILayout.Button("Clear"))
            {
                if (EditorUtility.DisplayDialog("Tip", "Want to delete all empty sprites, do you want to continue?", "Continue", "Cancel"))
                {
                    ...
                    {
                        string key = keys[i]; SpriteAsset.SpriteData data = component.Data[key];
                        if (data == null || data.sprite == null && IsDefaultKey(key))
                        {
        ...
        bool RendererDataMember(int index, string name, SpriteAsset.SpriteData data)
        {
            if (data == null)
            {
                data = component.Data[name] = new SpriteAsset.SpriteData();
            }
            ...
            {
                GUILayout.Label(data.sprite.texture, GUILayout.Width(50f), GUILayout.Height(50f));
            }
            else
            {
            ...
            //渲染精灵
            Sprite newSprite = EditorGUILayout.ObjectField(data.sprite, typeof(Sprite), false) as Sprite;
            if (newSprite != data.sprite)
            {
                data.sprite = newSprite;
                if (IsDefaultKey(name) && newSprite != null && !component.Data.ContainsKey(newSprite.texture.name))
                {
                    newName = newSprite.texture.name;
                    component.Data.Remove(name);
                    component.Data.Add(newName, data);
                    name = newName;
                }
